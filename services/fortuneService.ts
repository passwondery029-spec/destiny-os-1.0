...
import { calculateBazi, calculateDailyFortune as calculateRealDailyFortune, DailyFortune as RealDailyFortune, generateYearlyFortune } from './baziCalculator';
...
export function generateDailyFortune(userId: string, userBazi?: string, gender: string = 'MALE', birthDate?: string, birthTime?: string): DailyFortune {
    const todayDate = new Date();
    const todayStr = todayDate.toISOString().split('T')[0];
    const liuRi = getDailyBazi(todayDate);

    const seedString = `${userId}-${todayStr}-${userBazi || 'default'}`;
    const seed = hashString(seedString);
    const random = mulberry32(seed);

    let realFortune: RealDailyFortune | null = null;
    
    // 如果提供了生日信息，使用真实八字算法
    if (birthDate && birthTime) {
        try {
            const baziData = calculateBazi(birthDate, birthTime);
            realFortune = calculateRealDailyFortune(baziData, todayDate);
        } catch (error) {
            console.error('Error calculating real fortune:', error);
        }
    }

    const baziToUse = userBazi || "甲子 乙丑 丙寅 丁卯";
    const userElements = calculateBaziElements(baziToUse);
    const liuRiElements = [getElement(liuRi[0]), getElement(liuRi[1])];

    // 使用真实算法的分数或 fallback 到基础计算
    let score = realFortune?.overallScore || Math.floor(random() * 20) + 65;
    ...
    const brief = realFortune ? `流日【${liuRi}】，${realFortune.suggestion.substring(0, 15)}...` : `流日【${liuRi}】，${randomBrief}`;
    const advice = realFortune ? realFortune.suggestion : ADVICES[Math.floor(random() * ADVICES.length)];
    ...
    // 使用真实算法的五行数据或 fallback
    const elementsData: FiveElementsData[] = realFortune ? [
        { subject: '金 (专注)', A: realFortune.wealthScore, fullMark: 100 },
        { subject: '木 (成长)', A: realFortune.careerScore, fullMark: 100 },
        { subject: '水 (智慧)', A: realFortune.healthScore, fullMark: 100 },
        { subject: '火 (活力)', A: realFortune.relationshipScore, fullMark: 100 },
        { subject: '土 (稳重)', A: Math.floor((realFortune.wealthScore + realFortune.careerScore) / 2), fullMark: 100 },
    ] : [
    ...
    // 使用真实算法的月度数据或 fallback
    const energyCurve: EnergyPoint[] = realFortune ? realFortune.monthlyData.slice(0, 7).map((m, i) => ({
        time: ['卯时', '辰时', '午时', '申时', '酉时', '亥时', '子时'][i],
        energy: m.score
    })) : [
    ...
    return {
        ...
        energyCurve,
        notifText,
        luckyColorName: realFortune?.luckyColor || c1.name,
        luckyAction,
        liuRi
    };
}
